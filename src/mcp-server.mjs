...
export async function withWarpFallback(run, notify, deps = {}) {
  const { resolveProxy: getProxy = resolveProxy, readConfig: getConfig = readConfig, dockerAvailable: hasDocker = dockerAvailable, startWarp: warpStart = startWarp } = deps;
  ...
  try {
    ...
    const { proxy: warpProxy, ip } = await warpStart({ log: notify });
